 ...
 *       Filename:  test.c
 *
 *    Description:  tester for cthmclrx.
 *
 *        Version:  1.0
 *        Created:  2014/12/09 19时59分34秒
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  XadillaX Scarlet
 *   Organization:  Gencokyo
 *
 * =====================================================================================
 */
#include "../byakuren.h"
...
void print_stats(bkr_color_stats* stats, int count)
{
    ...
    {
        printf("{\"color\": %d, \"count\": %d}%c\n", stats[i].value, stats[i].count, i == count - 1 ? ' ' : ',');
    }
    printf("]\n");
    printf("}\n");
}
...
int main(int argc, char* argv[])
{
    ...
    bkr_rgb* rgb    = (bkr_rgb*)malloc(sizeof(bkr_rgb) * color_count);
    ...
    if(!strcmp("mindiff", argv[1]))
    {
        calc_mindiff(rgb, color_count);
    }
    else
    if(!strcmp("mix", argv[1]))
    {
